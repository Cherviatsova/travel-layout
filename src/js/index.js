...
scrollDownButton.addEventListener("click", (evt) => {
  ...
  startAnimationScroll(scrollToNode.offsetTop - 88);
});
...
function startAnimationScroll(newScrollY) {
  ...
  if (Math.abs(deltaScroll) > 1) {
    scrollAnimalionId = window.requestAnimationFrame(() =>
...
function stopAnimationScroll() {
  window.cancelAnimationFrame(scrollAnimalionId);
  scrollAnimalionId = undefined;
}
